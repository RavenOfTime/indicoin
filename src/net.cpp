...
static const char *strDNSSeed[][2] = {
    {"node.indicoin.karelia.pro", "dnsseed.indicoin.karelia.pro"},
    {"indicoin.ru", "dnsseed.indicoin.ru"},
    {"indicoin.ru", "testseed.indicoin.ru"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. indicoin is probably already running."), addrBind.ToString().c_str());
        else
