...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #indicoinTEST2\r");
            Send(hSocket, "WHO #indicoinTEST2\r");
        } else {
            // randomly join #indicoin00-#indicoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #indicoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #indicoin%02d\r", channel_number).c_str());
        }
