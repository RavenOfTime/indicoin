indicoin-qt: Qt4 GUI for indicoin
===============================
...
Alternatively, install Qt Creator and open the `indicoin-qt.pro` file.

An executable named `indicoin-qt` will be built.
...
A warning for people using the *static binary* version of indicoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of indicoin is linked against libdb5.3.
...
installed causes indicoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
